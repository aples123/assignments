...
def top5_countries_by_date(day: int, month: int, year: int = 2020) -> List[str]:
    ...
    df = df.sort_values(by=f"{month}/{day}/{year}", ascending=False)
    result = df['Country/Region'].head(5).tolist()
    return result
...
def no_new_cases_count(day: int, month: int, year: int = 2020) -> int:
    ...
    url = f"https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv"
    df = pd.read_csv(url, error_bad_lines=False)
    podany=datetime.strptime(str(day).zfill(2)+str(month).zfill(2)+str(year), '%d%m%Y')
    poprzedni = podany + timedelta(days=-1)
    podany = podany.strftime('%m/%d/%y').lstrip("0").replace("/0", "/")
    poprzedni = poprzedni.strftime('%m/%d/%y').lstrip("0").replace("/0", "/")
    df['roznica']= df[podany] - df[poprzedni]
    df = df.loc[df['roznica'] > 0]
    return df['Country/Region'].count()
